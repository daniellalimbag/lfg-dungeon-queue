 ...
 void QueueManager::start(int nInstances) {
   ...
   for (int i = 0; i < nInstances; ++i) {
     instances_.push_back(std::make_unique<DungeonInstance>(i));
   }
   running_ = true;
   ...
   while (running_) {
     ...
     int freeIdx = -1;
     {
       ...
       cv_.wait_for(lock, std::chrono::milliseconds(100), [this]{
         // wake periodically or when notified to attempt matching
         return !running_ || canFormParty_();
       });
       if (!running_) break;
       if (!canFormParty_()) continue;
       party = formParty_();
